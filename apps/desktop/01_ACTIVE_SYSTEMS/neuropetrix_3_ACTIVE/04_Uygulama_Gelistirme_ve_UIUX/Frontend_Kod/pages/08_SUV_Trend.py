...
if st.sidebar.button("🏠 Ana Sayfa", key="suv_nav_home", use_container_width=True):
    st.switch_page("streamlit_app.py")

if st.button("📊 Dashboard", key="suv_nav_dashboard", use_container_width=True):
    st.switch_page("pages/00_Dashboard.py")

if st.button("🤖 AI Analysis", key="suv_nav_ai", use_container_width=True):
...
with col_hero2:
    ...
    # Quick actions
    if st.button("📊 Trend Analizi", key="suv_trend_analyze", type="primary", use_container_width=True):
        st.session_state["trend_analysis"] = "auto"
        st.rerun()
    
    if st.button("📁 Veri Yükle", key="suv_upload", type="secondary", use_container_width=True):
...
# Add data button
if st.button("➕ SUV Verisi Ekle", key="suv_add", type="primary", use_container_width=True):
    new_data = {
...
with col_export1:
    ...
    if st.button("📥 İndir", key="suv_download", use_container_width=True):
...
with col_export2:
    ...
    if st.button("📝 Rapor Oluştur", key="suv_report", use_container_width=True):
        st.success("📝 Rapor oluşturuluyor...")

with col_export3:
    st.subheader("🏥 Klinik Entegrasyon")
    
    if st.button("🏥 HBYS'e Gönder", key="suv_send_hbys", use_container_width=True):
        st.info("🏥 HBYS entegrasyonu yakında eklenecek")
    
    if st.button("📧 E-posta Gönder", key="suv_send_email", use_container_width=True):
