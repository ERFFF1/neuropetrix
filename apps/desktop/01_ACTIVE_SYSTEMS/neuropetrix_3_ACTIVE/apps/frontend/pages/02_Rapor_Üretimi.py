...
with col_hero2:
    ...
    if st.session_state.get("current_patient"):
        ...
        # Quick actions
        if st.button("🚀 Hızlı Rapor", key="report_quick_create", type="primary", use_container_width=True):
            st.session_state["current_workflow_step"] = 3
            st.rerun()
    else:
        st.warning("⚠️ Önce hasta bilgilerini girin")
        if st.button("🏥 HBYS'e Git", key="report_go_hbys", type="primary"):
            st.switch_page("pages/03_HBYS_Entegrasyon.py")
...
*TSNM Formatında Dönüştürülmüş - NeuroPETrix AI Sistemi*
            ...
            if st.button("📊 AI Analysis'e Git", key="report_go_ai_analysis"):
