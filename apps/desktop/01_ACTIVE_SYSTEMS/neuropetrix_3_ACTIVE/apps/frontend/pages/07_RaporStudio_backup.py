...
with col_hero2:
    ...
    if st.button("🚀 Yeni Rapor", type="primary", use_container_width=True):
        st.session_state["report_studio_step"] = "input"
        st.rerun()
...
if st.session_state["report_studio_step"] == "input":
    ...
    with col_input1:
        ...
        if st.button("🚀 Ses Dosyasından Rapor Oluştur", type="primary", use_container_width=True):
            if uploaded_audio:
    ...
    with col_input2:
        ...
        if st.button("🤖 Demo Rapor Oluştur", use_container_width=True):
            st.session_state["report_studio_step"] = "processing"
            st.rerun()
    ...
    with col_action1:
        if st.button("🔄 Yeni Rapor", type="primary"):
            st.session_state["report_studio_step"] = "input"
            st.rerun()
