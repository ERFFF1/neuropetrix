...
with col_hero2:
    ...
    if st.button("🚀 Yeni PICO", type="primary", use_container_width=True):
        st.session_state["evidence_step"] = "pico"
        st.rerun()
    
    if st.button("🔎 Literatür Ara", use_container_width=True):
...
if st.session_state["evidence_step"] == "pico":
    ...
    st.write("")
    if st.button("🔎 Literatür Aramasına Geç", type="primary"):
        st.session_state["evidence_step"] = "search"
        st.rerun()
    ...
    with col_action1:
        if st.button("🔄 Yeni PICO", type="primary"):
            st.session_state["evidence_step"] = "pico"
            st.rerun()
    with col_action2:
        if st.button("📊 Dashboard", use_container_width=True):
    ...
    if st.button("📝 Raporu Sonlandır ve Kaydet", type="primary"):
    ...
    with col_export3:
        if st.button("📚 EndNote", use_container_width=True):
            st.info("📚 EndNote export özelliği yakında eklenecek...")
    
    with col_export4:
        if st.button("📝 Word Rapor", use_container_width=True):
